...
# --- STEP 3: DATABASE & ML INITIALIZATION ---
DATABASE_URL = os.environ.get("DATABASE_URL")
        ...
        if DATABASE_URL:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    """)
                    conn.commit()
            print("🗄️ Database tables verified.")
    except Exception as e:
...
def get_live_pune_temp():
    """Fetches real-time temp using Open-Meteo (No API Key Required)"""
    try:
        url = "https://api.open-meteo.com/v1/forecast?latitude=18.5204&longitude=73.8567&current=temperature_2m"
        response = requests.get(url, timeout=5).json()
...
async def save_query(payload: QueryPayload):
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO "PortfolioQueries" (location, budget) VALUES (%s, %s);',
                            (payload.location, payload.budget))
                conn.commit()
        return {"success": True}
    except Exception as e:
...
async def save_search_history(payload: QueryPayload, user_id: str = Depends(get_current_user_id)):
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute('INSERT INTO "SearchHistory" (user_id, location, budget) VALUES (%s, %s, %s);',
                            (user_id, payload.location, payload.budget))
                conn.commit()
        return {"success": True}
    except Exception as e:
...
async def get_search_history(user_id: str = Depends(get_current_user_id)):
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute('SELECT id, location, budget, createdAt, user_id FROM "SearchHistory" WHERE user_id = %s ORDER BY createdAt DESC;', (user_id,))
                history = cur.fetchall()
        return {"success": True, "history": history}
    except Exception as e:
        raise HTTPException(status_code=500, detail="Database error")
...
async def optimise(request: OptimiseRequest):
    ...
    COST_DICT = {
        "Cool Roofs": {"material": 1500000, "labor": 500000},
        "Urban Forest": {"material": 2000000, "labor": 1000000},
        "Permeable Rd": {"material": 7500000, "labor": 2000000},
        "Green Roofs": {"material": 10000000, "labor": 3500000}
    }
    ...
    # --- UPDATED OPTIMIZER LOOP ---
    print(f"💰 Starting Optimization for {request.area} with Budget: {remaining_budget}")
    
    for _, block in ranked_blocks.iterrows():
        # Clean the strategy string from the CSV
        raw_strategy = str(block.get('best_strategy', '')).strip()
        
        # FIX 1: If strategy is missing but the block has high risk, give it a default
        if not raw_strategy or raw_strategy.lower() in ['nan', 'none', '']:
            if float(block.get('gnn_vulnerability_pred', 0)) > 0.1:
                strategy = "Urban Forest" # Default fallback for high risk
            else:
                continue
        else:
        ...
        # FIX 2: Stop only when we have less than 5 Lakhs left
        if remaining_budget < 500000:
            break

    print(f"✅ Optimization complete. Blocks added to map: {len(portfolio)}")
